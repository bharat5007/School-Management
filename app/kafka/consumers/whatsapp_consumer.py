...
class WhatsAppNotificationConsumer(BaseNotificationConsumer):
    ...
    async def process_message(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Processing WhatsApp batch {whatsapp_payload.batch_id} with {len(whatsapp_payload.recipients)} recipients"
            )
            ...
            logger.info(
                f"WhatsApp batch {whatsapp_payload.batch_id} processed: {results}"
            )
    ...
    async def _process_whatsapp_batch(
        self, payload: BulkWhatsAppKafkaPayload
    ) -> Dict[str, int]:
        ...
        logger.info(
            f"Processing WhatsApp batch with conservative rate limiting (5/sec)"
        )
        ...
        for i, recipient in enumerate(payload.recipients):
            try:
                ...
                logger.error(
                    f"Failed to send WhatsApp to {recipient.get('whatsapp', 'unknown')}: {str(e)}"
                )
                results["failed"] += 1
    ...
    async def _send_whatsapp_message(
        self, recipient: Dict[str, Any], payload: BulkWhatsAppKafkaPayload
    ) -> bool:
        ...
        try:
            ...
            template_parameters = recipient.get(
                "template_parameters",
                payload.whatsapp_content.default_template_parameters or [],
            )

            logger.info(
                f"Sending WhatsApp to {whatsapp_number} for batch {payload.batch_id}"
            )
            ...
            await self._simulate_whatsapp_sending(
                whatsapp_number,
                payload.whatsapp_content.template_name,
                template_parameters,
                recipient_name,
            )
    ...
    async def _simulate_whatsapp_sending(
        self,
        whatsapp_number: str,
        template_name: str,
        parameters: List[str],
        ...
        logger.info(
            f"💬 WhatsApp sent to {whatsapp_number}{name_display}: Template '{template_name}'{params_display}"
        )
